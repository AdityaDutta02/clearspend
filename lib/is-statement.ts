...
export function scoreStatementText(text: string, regexTxnCount: number): StatementHeuristic {
  ...
  const vocabHits = (text.match(/\b(account statement|statement of account|account number|ifsc|opening balance|closing balance|available balance|transaction details|narration|debit|credit|withdrawal|deposit|upi)\b/gi) ?? []).length
